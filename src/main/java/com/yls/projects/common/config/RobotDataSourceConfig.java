...
@Configuration
@MapperScan(basePackages = RobotDataSourceConfig.PACKAGE, sqlSessionFactoryRef = "robotSqlSessionFactory")
public class RobotDataSourceConfig {

	// 精确到 szqx 目录，以便跟其他数据源隔离
	static final String PACKAGE = "com.yls.projects.*.dao";

	static final String MAPPER_LOCATION = "classpath:/mappings/robot/*/*.xml";

	static final String TYPE_ALIASES_PACKAGE = "com.yls.projects.*.entity";

	@Value("${robot.jdbc.url}")
	private String url;

	@Value("${robot.jdbc.username}")
	private String user;

	@Value("${robot.jdbc.password}")
	private String password;

	@Value("${robot.jdbc.driver}")
	...
	public SqlSessionFactory getSqlSessionFactory(@Qualifier("robotDataSource") DataSource dataSource)
			throws Exception {
